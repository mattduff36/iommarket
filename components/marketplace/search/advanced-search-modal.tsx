...
export function AdvancedSearchModal({
  ...
  const [fuelConsumptionRange, setFuelConsumptionRange] = React.useState<[number, number]>(
    parseBoundedRange(
      initial.minFuelConsumption,
      initial.maxFuelConsumption,
      FUEL_CONSUMPTION_MIN,
      FUEL_CONSUMPTION_MAX,
    ),
  );
  ...
  const [taxRange, setTaxRange] = React.useState<[number, number]>(
    parseBoundedRange(initial.minTax, initial.maxTax, TAX_MIN, TAX_MAX),
  );
  ...
  React.useEffect(() => {
    ...
    setTaxRange(parseBoundedRange(initial.minTax, initial.maxTax, TAX_MIN, TAX_MAX));
  ...
  function handleApply() {
    onApply({
      ...
      minPrice: rangeMinParam(priceRange[0], PRICE_MIN),
      maxPrice: rangeMaxParam(priceRange[1], PRICE_MAX),
      minMileage: rangeMinParam(mileageRange[0], MILEAGE_MIN),
      ...
      minTax: rangeMinParam(taxRange[0], TAX_MIN),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto lg:max-w-5xl">
        ...
        <div className="space-y-6 py-4 lg:grid lg:grid-cols-2 lg:items-start lg:gap-6 lg:space-y-0">
          ...
          <section>
            <h3 className="mb-3 text-sm font-semibold text-text-primary border-b border-border pb-2">Price, Mileage &amp; Year</h3>
            <div className="space-y-4">
              <RangeSlider label="Price" min={PRICE_MIN} max={PRICE_MAX} step={PRICE_STEP} value={priceRange} onValueChange={setPriceRange} formatValue={(v) => `£${v.toLocaleString()}`} scale="logarithmic" />
              <RangeSlider label="Mileage" min={MILEAGE_MIN} max={MILEAGE_MAX} step={MILEAGE_STEP} value={mileageRange} onValueChange={setMileageRange} formatValue={(v) => `${v.toLocaleString()} mi`} />
          ...
          <section>
            ...
            <div className="space-y-4">
              <RangeSlider label="Fuel Consumption" min={FUEL_CONSUMPTION_MIN} max={FUEL_CONSUMPTION_MAX} step={1} value={fuelConsumptionRange} onValueChange={setFuelConsumptionRange} formatValue={(v) => `${v} mpg`} />
              <RangeSlider label="CO2 Emissions" min={0} max={CO2_MAX} step={5} value={co2Range} onValueChange={setCo2Range} formatValue={(v) => `${v} g/km`} />
              <RangeSlider label="Tax Per Year" min={TAX_MIN} max={TAX_MAX} step={10} value={taxRange} onValueChange={setTaxRange} formatValue={(v) => `£${v}`} />
