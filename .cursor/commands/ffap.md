...
3. Run `npm run finalise:full:push`.
...
5. For a safely repairable step, inspect `private/automation/finalise-last-failure.json`, fix it, and run `npm run finalise:repair`. Repeat only that targeted check until stable, then run `npm run finalise:full:push` once for closure so the original push intent is preserved.
